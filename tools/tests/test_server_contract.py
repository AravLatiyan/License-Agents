...
def test_credential_phish_fails_auth_with_mismatched_link():
    ...
    assert "spf=fail" in result["authentication_results"]
    assert "dmarc=fail" in result["authentication_results"]
...
def test_invoice_fraud_bec_has_lookalike_domain_and_attachment():
    ...
    assert result["message_id"] == "<ceo-req-99231@universaI-imports.co>"
    assert result["from"] == "naitik.srivastava@universaI-imports.co"
    assert result["reply_to"] != result["from"]
    assert result["display_name"] == "Naitik Srivastava"
...
def test_legitimate_mail_matches_domains_throughout():
    ...
    domains = {
        result["from"].split("@")[1],
        result["reply_to"].split("@")[1],
        result["return_path"].split("@")[1],
    }
...
def test_cap_response_truncates_oversized_address_fields_under_the_2kb_cap():
    # Qodo's named "addresses" case - hostile from/reply_to strings, not a hostile list.
    ...
    capped = _cap_response(huge)
    assert capped["truncated"] is True
    assert len(capped["display_name"]) < 3000
    assert len(capped["reply_to"]) < 3000
    assert len(json.dumps(capped, ensure_ascii=False).encode("utf-8")) <= MAX_RESPONSE_BYTES


def test_cap_response_truncates_oversized_authentication_results_under_the_2kb_cap():
    huge = _minimal_result(authentication_results="spf=fail; " * 1000)
    capped = _cap_response(huge)
    assert capped["truncated"] is True
    assert len(capped["authentication_results"]) < len(huge["authentication_results"])
    assert len(json.dumps(capped, ensure_ascii=False).encode("utf-8")) <= MAX_RESPONSE_BYTES
...
def test_cap_response_handles_every_field_oversized_at_once():
    ...
    huge = {
        ...
        "authentication_results": "spf=fail (" + "h" * 6000 + ")",
