...
BLOCKED ON:   T-015 live-fire test — no model provider configured; T-023/T-024 runtime verification — actual delegation and reporting *behavior* unobserved, not just JSON acceptance; **`contracts/events.ts`'s `DomainIntel` type doesn't match `domain_intel()`'s real output — confirmed to fail `cockpit/src/missionSource.ts`'s `isDomainIntel` runtime check — must be fixed in `contracts/` before any live INFRASTRUCTURE evidence event carries `domain_intel` output** (§5/§8)
LAST UPDATED: 2026-08-27 19:02 · **PR #19 (the integration merge) opened, then a Qodo review pass fixed on top of it (§4).** PR #19 opened `tools/integrate-domain-intel` → `main`. Qodo's first review found 7 Bugs + 4 Rule violations — 6 fixed (real `parse_message`/`ParsedMessage` contract mismatch, MIME-tree URL extraction gap, `url_reputation`'s response-cap hang risk, `domain_intel`'s truncation-flag-ordering bug, malformed-`tags` crash risk, an ungated flaky live test, unvalidated domain syntax reaching RDAP), 3 deferred/declined with reasoning (the already-known `DomainIntel`/`UrlReputation` drift stays a separate `/contracts` PR; `.gitignore` documentation fixed via the PR description instead of code; the 400-line-PR rule restates the existing integration-merge exception). 25 new regression tests, 101/101 pass (1 correctly skipped — the newly-gated live RDAP/crt.sh test). Pushed to PR #19, not merged — waiting on Qodo to re-review the new HEAD. Prior integration-merge summary: real merge (not squash) of `tools/domain-intel` into `main` on branch `tools/integrate-domain-intel`, 19 files/2,041 lines purely additive, `PLAN.md` the only conflict (§1/§2/§4/§6/§7/§11, resolved by hand, every unique entry from both sides kept), verified clean before the original push (pytest 80/80, harness 13/13, contracts `tsc --strict` clean, cockpit build clean)
```
...
| `DomainIntel`/`UrlReputation` contract fix | O1/O2/O3 | Confirmed by direct inspection (not inferred) that `contracts/events.ts`'s `DomainIntel` type is a flat 4-field shape that `domain_intel()`'s real nested `{domain, rdap:{...}, cert:{...}, truncated}` output does not satisfy — `cockpit/src/missionSource.ts`'s `isDomainIntel` runtime check would reject it outright, throwing inside `assertMissionEvent` the first time a live INFRASTRUCTURE evidence event carries real `domain_intel` output. `UrlReputation` is narrower than reality but not currently breaking. First raised in §8 (2026-08-27); now checkable for real since `tools/domain-intel` merged into `main` (§4). **Independently re-confirmed by Qodo's PR #19 review** (finding #2, "Domain evidence fails validation") the same day — deliberately left unfixed in that PR's own Qodo-pass (§4) rather than flattening the tool, since `harness/agent.json`'s already-merged T-023/T-024 instructions deliberately target the tool's current real shape; Qodo's own review agreed this is a separate follow-up | 2026-08-27 | O1+O2 (or O1+O3): a focused `/contracts` PR updating `DomainIntel`/`UrlReputation` to match the real tool output (the `available`/`note`/`truncated` wrapper shape §6 already established as convention) — needs 2 approvals per CLAUDE.md's `/contracts` rule |
