...
| 3 | **T-034** | Wire the `imports-mcp` connector into `agent.json`'s `mcp_servers` (currently `[]` — no connector exists yet, gated or not) and configure `require_approval_for_tools: [quarantine, notify_impersonated, create_block_rule, file_abuse_report]` — the four sequential gates | O1 | — | T-025 disproven as an invalid task premise (see §7/§8) and pulled from this slot. Config-only, same `require_approval_for_tools` schema T-015 proved live end to end — but that was against a stub tool that existed on the target server; **none of the four gated tools exist yet** (`tools/imports_mcp/server.py` currently has only `parse_message`/`domain_intel`/`url_reputation`), so this can be written and schema-accepted now but its gates can't be meaningfully live-verified until T-030–T-033 (§11) land |
