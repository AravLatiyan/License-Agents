...
- [ ] **Qodo installed on the repo** — before the first real commit. *Cannot be retrofitted; the
      review trail is the entire evidence base for the Best Code Quality track*
...
|---|---|---|---|---|---|
| 1 | **T-001** | **SPIKE 1** — get headless chromium running in a Daytona sandbox and return a screenshot + redirect chain | O1 | 3h ⏱ | **Highest-risk unknown in the project.** Everything downstream depends on the answer |
| 2 | **T-002** | **SPIKE 2** — do TrueForge approvals carry structured payloads? Can 4 actions be granted individually? Does the HTTP API stream approval events? | O1 | 2h ⏱ | Decides the cockpit's core interaction; blocks Owner 3 when they join |
...
| T-002 approvals | **2h** | Agent *proposes* 4 actions → human ticks the ones they want in the cockpit → grants the set. Same UX, different plumbing |
...
- **Runs code in the sandbox:** deliberately detonates a stranger's payload — the sandbox is the
  *point*, not a feature bolted on for the rubric
...
```
  message arrives (IMAP / maildir watch)
   ...
   └────┬──────────────────────────────────────────┘
        │
   ┌────▼─────── 🔶 LICENCE REQUIRED ──────────────┐
   │ ☐ quarantine from other inboxes               │
   │ ☐ warn the person being impersonated          │
   │ ☐ push a block rule                           │
   │ ☐ file abuse report with registrar            │
   │        [grant each] [deny each + reason]      │
   └────┬──────────────────────────────────────────┘
        │
...
- **T-034** [O1] All four actions gated, each with a plain description of what it will do
- **T-035** [O1] Warm-sandbox or snapshot strategy — cold start is 2–5 min and kills the demo
- **T-036** [O3] LICENCE REQUIRED modal with **individual** grant/deny + reason field on deny
...
| 1:50–2:20 | 🔶 **LICENCE REQUIRED** — four actions. Grant three, **deny one** with a reason |
