...
DATE:         2026-08-27
DAYS LEFT:    3
ACTIVE:       Owner 1 (Harness) · Owner 2 (Tools) · Owner 3 (Cockpit) · Owner 4 (Mission, PR #22 open — see LAST UPDATED)
DORMANT:      nobody currently — confirmed against live GitHub PR state, not assumed from PLAN.md's own prior text
TODAY'S GOAL: Slice 1 running end to end, ugly (O1) — three subagents now defined in agent.json, instructed to report structured evidence per T-024 (prompt guidance merged, runtime behavior still unverified — see BLOCKED ON). `tools/domain-intel` (parse_message, normaliser, domain_intel, url_reputation — T-003/T-004/T-010/T-011/T-012/T-020/T-021, all O2) now merged into `main` (see LAST UPDATED) — Slice 1/2's O2 read-only tool work is fully integrated; `correspondence_history` (T-022) is the one still outstanding (§2), now with a real dependency on T-060 recorded in §5. Contracts + Cockpit scaffold (T-050) already on main, consuming the mission fixture end to end (O3). T-060 (Range: Mailpit + fake portal + seed.sh) has an open PR (#22, O4) not yet merged.
BLOCKED ON:   T-015 live-fire test — no model provider configured; T-023/T-024 runtime verification — actual delegation and reporting *behavior* unobserved, not just JSON acceptance; T-022 — needs an owner decision on the T-060 dependency (§5)
LAST UPDATED: 2026-08-27 23:41 · **PLAN.md sync only, no code change — cross-owner state check before starting T-022.** Queried live GitHub PR state directly (not assumed from PLAN.md's own text): confirmed `main` is unchanged since the last sync (still `fdba4de`+PR#21's `25c6c4b`, T-020/T-021 fully merged, `DomainIntel` mirror fix intact) — nothing new there. Found two other owners' PRs PLAN.md didn't know about: **PR #22** (`mission/t060-range` → `main`, open, mergeable, O4) adds `range/docker-compose.yml`, `range/fake-portal/{Dockerfile,server.py}`, `range/mailpit-api.json` (Mailpit's own vendored OpenAPI spec, 2130 lines), `range/seed.sh` — read directly off that branch (not merged, so not on `main` yet): the compose file exposes Mailpit's HTTP API/UI (`8025`) and SMTP (`1025`) only, no IMAP port anywhere, and `seed.sh` seeds fixtures via Mailpit's own `POST /api/v1/send` HTTP endpoint, not SMTP delivery or IMAP. This is real, sourced fact from the actual PR diff, not an assumption about Mailpit in general. **PR #20** (`docs/t025-invalid-premise` → `main`, open, `mergeable: false` — behind `main` by 1 commit since PR #21 landed, needs a rebase) argues T-025 is schema-invalid (`dynamic_sub_agents` only accepts `{enabled: bool}`, no per-subagent model field exists) and proposes removing it from §2/§11 in favor of T-034. **Not adopted here** — that's O1's call to land through their own PR, not something to fold into a docs-sync commit; §2's T-025 row is untouched. New §5 row records the T-022↔T-060 dependency explicitly (see there) rather than guessing at a Mailpit IMAP configuration that doesn't exist in any merged or open PR. Verified before writing: `pytest` from repo root — 104 passed, 1 skipped, unchanged
```
...
> Note (2026-08-27, cross-owner sync): before picking **T-022**, read the new §5 row — it has a real, unresolved dependency on T-060 (O4's Range/Mailpit PR #22, unmerged) that changes what "IMAP search" in this row's own description actually means. Also: an open, unmerged PR (#20) argues **T-025** is schema-invalid and should be removed in favor of T-034 — not adopted into this row, that's O1's PR to land, flagged here only so nobody picks T-025 assuming it's uncontested.
...
| T-022 ↔ T-060 dependency/ownership | O2/O4 | T-022's own description ("`correspondence_history` — IMAP search for prior contact") never named which mailbox. §13 Tier 1 says the Range (Mailpit) is what judges actually run, which makes it the right target — but T-060 (the Range's Mailpit setup) is O4's task, still unmerged (PR #22, `mission/t060-range`), and its `docker-compose.yml` (read directly off that branch) exposes only Mailpit's HTTP API (`8025`) and SMTP (`1025`) — **no IMAP port at all**. `T-022`'s own task name ("IMAP search") may not even match what PR #22 will actually expose once merged; the vendored `range/mailpit-api.json` documents Mailpit's HTTP `/api/v1/` search endpoints instead, which is what's actually reachable today. Nobody has decided: (a) does T-022 wait for T-060 to merge and, if so, does that PR need an IMAP port added first, or (b) does `correspondence_history` target Mailpit's HTTP API instead of real IMAP, which would make T-022's own name stale too | 2026-08-27 | O2 (owns T-022) and O4 (owns T-060/PR #22) need to agree which protocol `correspondence_history` actually speaks against Mailpit, and whether that's a change to PR #22 (add an IMAP port) or to T-022's own scope (HTTP API client, not an IMAP client) — record the decision in §6 once made, this row is intentionally not deciding it |
...
- **T-060** Range: `docker compose up` → Mailpit + fake portal + `seed.sh` — **PR #22 open (2026-08-27), not yet merged; §5 has the T-022 dependency this creates**
