...
TODAY'S GOAL: Slice 1 running end to end, ugly (O1) — three subagents now defined in agent.json. Intel sources + MCP transport settled, normaliser starting (O2); T-020 (domain_intel) and T-021 (url_reputation) both now fully complete on the `tools/` stack — all three planned imports-mcp Slice 2 read-only tools (`parse_message`, `domain_intel`, `url_reputation`) done. Contracts merged into main, ready for Cockpit to build against (O3).
BLOCKED ON:   T-015 live-fire test — no model provider configured; T-023 runtime verification — actual delegation *behavior* unobserved, not just JSON acceptance (§5)
LAST UPDATED: 2026-08-26 23:32 · PR #1-#5 and PR #12 (T-016) all merged into main (T-013/T-017/T-002/T-014+Qodo-fixes/T-015+Qodo-fixes/T-003/T-004/T-016+Qodo-fixes all on main) — **correction while resolving this conflict:** both sides' prior text still said PR #12 "needs a 2nd human approval," but `gh pr view 12` confirms it merged at 2026-08-26T10:03:59Z as `cbe5d52`, before either side's own last update; T-016 (PR #12) removed from BLOCKED ON accordingly. T-023 (PR #13) — Qodo found real doc-overclaiming (dynamic_sub_agents doesn't enforce named/scoped subagents), fixed, being merged now (this PR). T-020 (domain_intel) done — PR #9 (`1aebf464`) + PR #14 (`5ca7fa53`) both merged into `tools/domain-intel`, all Qodo findings resolved/dismissed with cited evidence, 59/59 tests pass; T-021 (url_reputation) done — PR #10 rebased onto `tools/domain-intel` and merged as `766d3052`, all 4 pre-rebase Qodo findings fixed, but Qodo never produced a review for the exact final HEAD (`30b8bd7`) despite the rebase, a force-push, and a base retarget — merged via an explicit one-time authorized exception to the Qodo-before-merge rule (§6), after independently re-verifying mergeable/clean, 15/15 T-021 tests, 24/24 T-020 regression tests, 80/80 full suite, and a diff scoped to T-021's own files only; `tools/domain-intel` (now including both T-020 and T-021) not yet merged up into `main`; live-fire still blocked on a model provider key
```
...
|---|---|---|---|---|---|
| 1 | **T-050** | Cockpit scaffold + event-stream consumer, rendering `contracts/fixtures/` start to finish | O3 | — | T-016 is confirmed merged into `main` (`cbe5d52`) — genuinely unblocked now, not just "pending approval." Nothing in the Cockpit slice can start until this exists |
...
| 3 | **T-024** | Prompt subagents to return **structured evidence, not prose**; narrow remits so they don't duplicate work | O1 | — | Direct follow-on to T-023 (just merged) — the three subagents exist, but nothing yet enforces their output shape |
...
| T-023 runtime verification | O1 | The `instructions` field is accepted-schema-safe (same as T-013/T-017), but that's not the open question — the open question is *behavior*: whether a real turn actually spawns three subagents matching these names/remits, and whether an IDENTITY-labeled one stays tool-free in practice, given `dynamic_sub_agents` doesn't enforce either (§4, Qodo pass) | 2026-08-26 | Whoever next has TrueForge running locally + a model provider key: run a real turn against a suspicious-email fixture and observe the actual `create_sub_agent` calls/names/tool use, not just POST `harness/agent.json` and check the status code |
