...
LAST UPDATED: 2026-08-27 18:22 · **Integration: `tools/domain-intel` merged into `main`** on branch `tools/integrate-domain-intel` (real merge, not squash — preserves T-020/T-021's individual Qodo-reviewed commit history). Read-only per file/tree inspection first (`git merge-tree --write-tree`): confirmed the only conflicting file would be `PLAN.md`, confirmed zero modifications to any file `main` already owned (`contracts/`, `cockpit/`, `harness/`, `README.md`, `CLAUDE.md` all untouched), confirmed `main` had **zero** `/tools` code before this merge — the whole O2 stack existed only on `tools/domain-intel` until now. Added: `tools/imports_mcp/{__init__,normaliser,server,domain_intel,url_reputation}.py`, `tools/tests/*` (8 files), `tools/fixtures/*.eml` (3 files), `tools/requirements.txt`, root `pytest.ini` — 19 files, 2,041 lines, purely additive. `PLAN.md` was the only real conflict, spanning §1/§2/§4/§6/§7/§11 (both branches had edited it independently since the shared merge-base, PR #3); resolved by hand preserving every unique entry from both sides — no line dropped, duplicates (T-020/T-021 entries already docs-synced onto `main` by an earlier `docs/plan-sync-t020` branch) collapsed to one copy each. **Verification before push:** `pytest` from repo root — 80/80 pass; `test_server_integration.py` run in isolation — 6/6 pass (the known pre-existing flakiness, §7, didn't reproduce this run); `harness`: `npm test` — 13/13 pass; `contracts`: `tsc --noEmit --strict events.ts events.typecheck.ts` — clean; `cockpit`: `npm run build` — clean. **Also confirmed, not fixed here:** a compatibility review before this merge (read-only) traced the already-logged `DomainIntel`/`UrlReputation` contract drift (§8, 2026-08-27) all the way to `cockpit/src/missionSource.ts`'s runtime validator — `domain_intel()`'s real nested `{domain, rdap:{...}, cert:{...}, truncated}` output does not satisfy `isDomainIntel`'s flat-field check at all, so any live mission event carrying it would throw inside Cockpit today. `url_reputation()`'s real output happens to satisfy `isUrlReputation` (extra fields ignored), so that half is not currently broken, just under-modeled. Fix belongs in `contracts/events.ts` (update the type to match the real tool output, per §8's own recommended `available`/`note` wrapper), not in the Python tools — `agent.json`'s T-023/T-024 instructions already deliberately target the real tool shape, not the stale contract. Not touched in this PR by explicit scope (`/contracts` needs 2 approvals; a separate follow-up PR is next). T-022 intentionally not started here either. PR pending open against `main`
```
...
| `DomainIntel`/`UrlReputation` contract fix | O1/O2/O3 | Confirmed by direct inspection (not inferred) that `contracts/events.ts`'s `DomainIntel` type is a flat 4-field shape that `domain_intel()`'s real nested `{domain, rdap:{...}, cert:{...}, truncated}` output does not satisfy — `cockpit/src/missionSource.ts`'s `isDomainIntel` runtime check would reject it outright, throwing inside `assertMissionEvent` the first time a live INFRASTRUCTURE evidence event carries real `domain_intel` output. `UrlReputation` is narrower than reality but not currently breaking. First raised in §8 (2026-08-27); now checkable for real since `tools/domain-intel` merged into `main` (§4) | 2026-08-27 | O1+O2 (or O1+O3): a focused `/contracts` PR updating `DomainIntel`/`UrlReputation` to match the real tool output (the `available`/`note`/`truncated` wrapper shape §6 already established as convention) — needs 2 approvals per CLAUDE.md's `/contracts` rule |
