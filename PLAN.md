...
| 3 | **T-035** | Warm-sandbox or snapshot strategy, **and** actually wire `detonate.js` to execute inside the Daytona sandbox job | O1 | — | T-034 done and merged (§4) — swapped for the next O1 item in §11's order. `detonate.js` still has zero Daytona integration (§8, 2026-08-25) and cold start is 2–5 min, which trap #13 says is a demo problem, not a capability one |
